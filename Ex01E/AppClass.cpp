...
#pragma endregion
	...
	float percentage;//our percentage from start position to end position
	float timeBetweenStops; //time between stops
	static int route = 0;

	start = m_v3StopList[route];
	end = m_v3StopList[(route + 1) % m_v3StopList.size()];

	//set timing between stops
	timeBetweenStops = 2.0f; //2 second timing
	percentage = MapValue(fTimer, 0.0f, timeBetweenStops, 0.0f, 1.0f);




	m_sProgrammer = "Kyle Lekkas - knl1637@rit.edu"; //Replace with your name and email
	...
	if (percentage >= 1.0f) {
		...
		route %= m_v3StopList.size(); //check if in boundaries
	}
