...
void MyMesh::GenerateCone(float a_fRadius, float a_fHeight, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	//Get sections of cone
	float step = (PI * 2) / a_nSubdivisions;
	float currentAngle = step;

	//for each section, draw a line around the cone
	for (int i = 0; i < a_nSubdivisions; i++) {
		//get next points in line
		newX = cos(currentAngle) * a_fRadius;
		newY = sin(currentAngle) * a_fRadius;

		//base
		AddTri(vector3(currentX, 0, currentY), vector3(0, 0, 0), vector3(newX, 0, newY));

		//top, which is base but center point is extruded out
		AddTri(vector3(newX, 0, newY), vector3(0, -a_fHeight, 0), vector3(currentX, 0, currentY));

		//set our current to our new
		currentX = newX;
		currentY = newY;
